...
let test =
  let _test_trigo_sinus =
    ...
    let () = Assert.assert (Rational.lt diff error_threshold) in
    // sin(PI/2)
    ...
    let () = Assert.assert (Rational.lt diff error_threshold) in
    Test.Next.IO.log ("Test 'trigo sinus (with rational)' finished") in
  let _test_trigo_cosinus =
    ...
    let () = Assert.assert (Rational.lt diff error_threshold) in
    Test.Next.IO.log ("Test 'trigo cosinus (with rational)' finished") in
  let _test_trigo =
    let error_threshold =
      (Rational.inverse (Rational.new (int (Math.power (10n, 12n))))) in
    let precision : nat = 11n in
    ...
    let () = Assert.assert (Rational.lt diff error_threshold) in
    Test.Next.IO.log ("Test 'trigo (with rational)' finished") in
  ()
